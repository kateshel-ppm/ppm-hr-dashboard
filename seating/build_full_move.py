...
Условия:
  * переезжают все, кроме офис-менеджеров (АХО остаются на Остоженке);
...
PLAN = [
    ...
    # 345а — кабинет с перегородкой: генеральный директор, вакансии
    # операционного директора и директора по орг развитию, MAZE вдвоём
    # (как и сейчас в П7).
    ("345а", [C["Генеральный директор"], C["Операционный директор"],
              C["Директор по орг развитию"]] + take("MAZE")),
]
...
put([f"Рабочих мест по плану {total_cap} (по стульям у столов). Переезжает {seated}: "
     ...
     f"Свободно {total_cap - seated}. Офис-менеджеры остаются на Остоженке, ВЭД вне рассадки. "
...
put(["Не переезжают", len(staying), "", "",
     "офис-менеджеры (АХО): " + ", ".join(r[FIO].strip() for r in staying)], "grey")
put(["Не занимаем", 0, "", "",
...
for r in staying:
    put([r[DEP].strip(), who(r), r[POS].strip(), "", "офис-менеджеры не переезжают"], "grey")
