...
Условия:
  * не переезжают АХО, MAZE, ВЭД и люди из списка заказчика (STAY_NAMES);
...
MARKETING_STAYS = "--marketing-stays" in sys.argv
TAB = ("Рассадка — новый офис (без маркетинга)" if MARKETING_STAYS
...
staying = [r for r in need_all if stays(r)]
need = [r for r in need_all if not stays(r)]
missing = STAY_NAMES - {r[FIO].strip() for r in staying}
assert not missing, f"в списке остающихся не найдены в штате: {missing}"
...
PLAN = [
    ...
    # 349 — маркетинг с директором по маркетингу; Ермохин (по штатке PR,
    # по решению заказчика работает с маркетингом) сидит здесь же.
    ("349", [] if MARKETING_STAYS
            else [C["Директор по маркетингу"]] + take("Маркетинг") + take("PR")),
    # 347 — коммерческий блок без директора: партнёры, продажи, рефералы
    # и вакансия операционного директора. Малый кабинет как раз на них.
    # В варианте 2 коммерческий блок остаётся целиком, а вакансия
    # операционного директора уходит в кабинет с перегородкой 345а.
    ("347", [] if MARKETING_STAYS else
            take("Партнеры") + take("Продажи") + take("Рефералы")
            + [C["Операционный директор"]]),
    # 346 (и 345а в варианте 1) остаются пустыми — резерв под рост.
    ("346", []),
    ("345а", [C["Операционный директор"]] if MARKETING_STAYS else []),
]
...
put(["РАССАДКА — НОВЫЙ ОФИС, ВАРИАНТ 2: МАРКЕТИНГ ОСТАЁТСЯ (кабинеты 345–349)"
     if MARKETING_STAYS else
     "РАССАДКА — НОВЫЙ ОФИС, ПОЛНЫЙ ПЕРЕЕЗД (кабинеты 345–349)"], "title")
put([f"Рабочих мест по плану {total_cap} (по стульям у столов). Переезжает {seated}: "
     ...
     f"Свободно {total_cap - seated}. На Остоженке остаются {len(staying)} (список заказчика: "
     f"руководство, финансы, юристы, PR, офис-менеджеры, MAZE, ВЭД"
     f"{', маркетинг целиком' if MARKETING_STAYS else ''}). "
...
stay_by_dep = defaultdict(list)
for r in staying:
    stay_by_dep[r[DEP].strip()].append(r)
put(["Не переезжают", len(staying), "", "",
     " · ".join(f"{d} ({len(rs)})" for d, rs in
                sorted(stay_by_dep.items(), key=lambda x: -len(x[1])))
     + " — см. раздел 5"], "grey")
put(["Не занимаем", 0, "", "",
...
for d, rs in sorted(stay_by_dep.items(), key=lambda x: -len(x[1])):
    for r in sorted(rs, key=lambda r: (vac(r), r[FIO].strip())):
        put([d, who(r), r[POS].strip(), "", why_stays(r)],
            "amber" if vac(r) else "grey")
